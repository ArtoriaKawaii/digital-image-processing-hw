...
def showRGBin24_bit(num_of_RGB):
    ...
    if channels == 3:
        ...
        gray2RGB = np.asarray(Image_for_processing)
        split_img[ :, :, num_of_RGB] = (np.swapaxes(gray2RGB,0,1))[ :, :]
        split_img = np.swapaxes(split_img,0,1)
...
def color_complements():
    ...
    width, height = Image_for_processing.size
    pixel_values = np.array(Image_for_processing)
    
    # create zero matrix
    # 'dtype' by default: 'numpy.float64'
    split_img = np.zeros((height,width,3), dtype="uint8") 
    # assing each channel
    split_img[ :, :, 0] = 255-pixel_values[ :, :, 0]
    split_img[ :, :, 1] = 255-pixel_values[ :, :, 1]
    split_img[ :, :, 2] = 255-pixel_values[ :, :, 2]

    show_result(Image.fromarray(split_img))

def Laplacian_sharp(degree):

    kernel_array = [ [ -1 for i in range(degree) ] for j in range(degree) ]
    # print(kernel_size)
    # set center to degree*degree
    kernel_array[int((degree+1)/2-1)][int((degree+1)/2-1)] = degree*degree
    kernel = np.array(kernel_array)

    # print(kernel)
    # applying the sharpening kernel to the input image
    smooth = cv2.blur(np.array(Image_for_processing), (5, 5))
...
def segment():
    ...
    image_np = np.asarray(Image_for_processing)
    plt = cv2.cvtColor(np.asarray(image_np),cv2.COLOR_RGB2BGR)
    HSV = cv2.cvtColor(plt, cv2.COLOR_BGR2HSV)
    # decide purple range in HSV
    Purple_lowerbound = np.array([125, 43, 46])
    Purple_Upperbound = np.array([155, 255, 255])

    # check in range or not
    mask = cv2.inRange(HSV, Purple_lowerbound, Purple_Upperbound)
    Purple_Things = cv2.bitwise_and(plt, plt, mask = mask)
    Purple_Things = cv2.cvtColor(Purple_Things, cv2.COLOR_HSV2BGR)

    result = Image.fromarray(cv2.cvtColor(Purple_Things,cv2.COLOR_BGR2RGB))
    show_result(result)
...
# labals and scale for Laplacian_sharp
kernel_size = 5
btn_splitLap = tk.Button(mainWindow, text = "Laplacian_sharp", font = 24 , command = lambda:Laplacian_sharp(kernel_size))
btn_splitLap.place(x = 450 ,y = 600, width = 125 ,height = 30)

# labals and scale for complements
btn_complements = tk.Button(mainWindow, text = "complements", font = 24 , command = color_complements)
btn_complements.place(x = 600 ,y = 600, width = 125 ,height = 30)

# labals and scale for segment
btn_complements = tk.Button(mainWindow, text = "segment", font = 24 , command = segment)
btn_complements.place(x = 50 ,y = 700, width = 125 ,height = 30)
